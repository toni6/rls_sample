defmodule RlsSampleWeb.UserLive.Registration do
  ...
  def mount(_params, _session, %{assigns: %{current_scope: %{user: user}}} = socket)
    ...
    {:ok, redirect(socket, to: RlsSampleWeb.UserAuth.signed_in_path(socket))}
  end

  def mount(_params, _session, socket) do
    companies = Accounts.list_companies()
    company_options = Enum.map(companies, &{&1.name, &1.id})
